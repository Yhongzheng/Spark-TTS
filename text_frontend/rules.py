...
# ≥8位编号逐字（避免前后紧邻字母/数字时被识别）
RE_LONGNUM = re.compile(r"(?<![A-Za-z0-9])(\d{8,})(?!\d)")
# 11位手机号（同样避免前面是字母/数字时误匹配）
RE_PHONE11 = re.compile(r"(?<![A-Za-z0-9])(1\d{10})(?!\d)")
RE_PURENUM = re.compile(r"(?<![A-Za-z0-9])(\d+(?:\.\d+)?)(?!\d)")  # 普通纯数字/小数
